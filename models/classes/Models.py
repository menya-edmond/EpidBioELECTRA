...
class PubmedBERT(nn.Module):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.bert = AutoModelForMaskedLM.from_pretrained("microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract")
        self.output = nn.Linear(self.bert.config.hidden_size, 2)
        self.softmax = nn.Softmax(dim=1)
...
class TransformerXL(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        encoded_layer = self.bert(input_ids=input_ids,output_hidden_states=True)
        encl = encoded_layer[-1][-1]
        max_pooled, _  = torch.max(encl, 1)
        out = self.output(max_pooled)
