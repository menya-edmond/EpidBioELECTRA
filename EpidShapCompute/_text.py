...
class Text(Masker):
    ...
    def __init__(self, tokenizer=None, mask_token=None, collapse_mask_token="auto", output_type="string"):
        ...
        # cache variables
        self._s = None
        self.tokenized_s_full = None
        self.tokenized_s = None
        self._segments_s = None
        self._attention_mask_s = None #addd by me for attention mask return
    ...
    def __call__(self, mask, s):
        if type(s) is torch.Tensor:
          s=s.tolist()
          #print("first s before: ",s,"\n")
          #print("s is a Tensor and not string")
          s=self.tokenizer.convert_tokens_to_string(self.tokenizer.convert_ids_to_tokens(s))
          #print("s after: ",s,"\n")
        """else:
          print("first s is just a normal string")
          print("s: ",s,"\n")"""
        ...
        if self.output_type == "string":
            ...
            if self.mask_token_id is None:
                out = self.tokenized_s[0][mask]
            else:
                out = np.array([self.tokenized_s[0][i] if mask[i] else self.mask_token_id for i in range(len(mask))])
        ...
        # for more details.
        #return (np.array([out]),)
        return (np.array([out]),self._attention_mask_s)
    ...
    def token_segments(self, s):
        ...
        if type(s) is torch.Tensor:
          s=s.tolist()
          #print("second s before: ",s,"\n")
          #print("s is a Tensor and not string")
          s=self.tokenizer.convert_tokens_to_string(self.tokenizer.convert_ids_to_tokens(s))
          #print("s after: ",s,"\n")
        """else:
          print("second s is just a normal string")
          print("s: ",s,"\n")"""
        try:
            token_data = self.tokenizer(s,padding='max_length',max_length=128, return_offsets_mapping=True)
            ...
            return parts, token_data["input_ids"], token_data["attention_mask"]
    ...
    #     """
    #     M = len(self.tokenized_s[0])
    #     assert len(clustering)+1 == M
    #     def recursive_mark(ind):
    #         if ind < M:
    #             return list(self.tokenized_s[0][ind:ind+1])
    ...
    def _update_s_cache(self, s):
        if type(s) is torch.Tensor:
          s=s.tolist()
          #print("third s before: ",s,"\n")
          #print("s is a Tensor and not string")
          s=self.tokenizer.convert_tokens_to_string(self.tokenizer.convert_ids_to_tokens(s))
          #print("s after: ",s,"\n")
        """else:
          print("third s is just a normal string")
          print("s: ",s,"\n")"""
        if self._s != s:
            self._s = s
            tokens, token_ids, attention_mask = self.token_segments(s)
            self.tokenized_s = np.array([token_ids])
            self._attention_mask_s = np.array([attention_mask])
